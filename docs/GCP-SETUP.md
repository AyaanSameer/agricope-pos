...
```bash
cd "/Applications/Agricope/POS System" && gcloud builds submit --tag me-central1-docker.pkg.dev/YOUR_PROJECT_ID/agricope/pos:v0.1.0
```

Deploy it next to the database, with the three secrets injected:
...
- `--add-cloudsql-instances` mounts a Unix socket at `/cloudsql/CONNECTION_NAME`,
  so the API reaches Postgres over Google's internal network and the database
...
Both scripts talk to the production database through the Cloud SQL Auth
...
- One Cloud SQL instance per environment. A staging instance and service,
  deployed from `main`, catch problems before a tag reaches production.
...
Cloud SQL on `db-g1-small` with 20 GB is the steadier line item. Confirm
current `me-central1` figures on the
